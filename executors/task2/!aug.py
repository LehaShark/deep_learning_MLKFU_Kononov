...
from configs import MNISTConfig, MLPConfig
...
def main(dataset_config, model_config):
    ...
    for i_epoch in range(model_config.epochs):
        epoch_manager.step(train_dataloader, is_train=True, show_each=100, i_epoch=i_epoch)

    with tensor.no_grad(model):
        epoch_manager.step(test_dataloader, is_train=False, show_each=100)
